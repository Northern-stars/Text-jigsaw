"""Train a direct permutation-classification solver for MangaZero panels."""
...
from Solver.agent.mangazero_permutation_classifier import (
    MangaZeroPermutationClassifier,
    permutation_cross_entropy,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train MangaZero panel ordering as direct permutation classification.")
    ...
    parser.add_argument("--save-dir", type=Path, default=Path("Solver/checkpoints_mangazero_permutation_classifier"))
...
def main() -> None:
    ...
    model = build_model_from_args(args, use_layout).to(device)
...
def make_loader(dataset: Subset, args: argparse.Namespace) -> DataLoader | None:
    if len(dataset) == 0:
        return None
    return DataLoader(
        dataset,
...
def build_model_from_args(args: argparse.Namespace, use_layout: bool) -> MangaZeroPermutationClassifier:
...
def train_one_epoch(
    model: MangaZeroPermutationClassifier,
    loader: DataLoader,
    ...
    for batch in progress:
        ...
        loss = permutation_cross_entropy(logits, batch["target_order"], model)
    ...
    torch.save(
        {
            ...
            "epoch": epoch,
            "config": vars(args),
            "solver_family": "permutation_classifier",
