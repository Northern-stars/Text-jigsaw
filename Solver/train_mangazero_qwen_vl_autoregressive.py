"""Train the Qwen2.5-VL + LoRA autoregressive pointer solver."""
...
from Solver.agent.mangazero_qwen_vl_autoregressive import (
    MangaZeroQwenVLAutoregressiveSolver,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Qwen2.5-VL + LoRA autoregressive pointer solver.")
    ...
    parser.add_argument("--save-dir", type=Path, default=Path("Solver/checkpoints_mangazero_qwen_vl_autoregressive"))
...
def build_model(args: argparse.Namespace) -> MangaZeroQwenVLAutoregressiveSolver:
    target_modules = tuple(
        item.strip()
        for item in args.lora_target_modules.split(",")
        if item.strip()
    )
    return MangaZeroQwenVLAutoregressiveSolver(
...
def build_optimizer(
    model: MangaZeroQwenVLAutoregressiveSolver,
    ...
    if decoder_params:
        groups.append({"params": decoder_params, "lr": args.lr_decoder})
...
def train_one_epoch(
    ...
    for step, batch in enumerate(progress, start=1):
        ...
        loss = pointer_cross_entropy(logits, batch["target_order"]) / args.grad_accum_steps
        loss.backward()
